...
namespace Physics {
    ...
    struct Force {
        using filter_t = bool (*)(const SimulationCircle&);
        using calc_t = std::function<glm::vec2(const SimulationCircle&)>;

        bool is_affecting(const SimulationCircle& item) { return m_filter(item); }
        glm::vec2 calculate (const SimulationCircle& item) { return m_calc(item); }

        calc_t m_calc;
        filter_t m_filter = [](const SimulationCircle&) { return true; };
    };
    ...
    inline Force universal_gravitation(float mass, glm::vec2 source, float gravitational_constant = c_gravitational_constant) {
        auto calc = [=](const SimulationCircle& item) { 
    ...
    inline Force earth_gravitation() {
        return { [](const SimulationCircle& item) -> glm::vec2 { return { 0.0f, 9.8f * item.mass }; } };
    }
