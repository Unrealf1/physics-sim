...
namespace Physics {
    ...
    PhysicsItem SemiImplicitEuler::update(float dt, const PhysicsItem& item) {
        //TODO: test for speed (use of execution policies or threads)
        assert(("Mass is sane", item.mass > 0));
        ...
        glm::vec2 acceleration = total_force / item.mass;
        glm::vec2 new_speed = item.speed + dt * acceleration;
        glm::vec2 new_position = item.position + dt * new_speed;
        float rotational_acceleration = total_torque / item.inertia;
        float new_rotation_speed = item.rotation_speed + dt * rotational_acceleration;
        float new_orientation = item.orientation + dt * new_rotation_speed;
