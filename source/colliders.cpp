...
bool CircleCollider::is_colliding(const SimulationCircle& other) const {
    auto diff = other.position - m_position;
    auto square_dist = diff.x * diff.x + diff.y * diff.y;
    auto rad_sum = other.radius + m_radius;
...
bool StaticSegmentCollider::is_colliding(const SimulationCircle& circle) const {
...
glm::vec2 StaticSegmentCollider::collision_point(const SimulationCircle& circle) const {
    auto [t1, t2, ts_exist] = get_ts(circle);
...
std::tuple<float, float, bool> StaticSegmentCollider::get_ts(const SimulationCircle& circle) const {
    glm::vec2 delta = m_start - circle.position;
    float a = m_diff.x * m_diff.x + m_diff.y * m_diff.y;
    float b = 2.0f * (delta.x * m_diff.x + delta.y * m_diff.y);           
    float c = delta.x * delta.x + delta.y * delta.y - circle.radius * circle.radius;    
    float D = b * b - 4 * a * c;
    if (D < 0.0f) {
...
bool StaticCircleCollider::is_colliding(const SimulationCircle& circle) const {
...
glm::vec2 StaticCircleCollider::collision_point(const SimulationCircle& circle) const {
    auto dp = m_collider.m_position - circle.position;
    return circle.position + glm::normalize(dp) * circle.radius;
}
